...
if($user['position_id'] == '24'){?>
    ...
    $documentos = $conexion2 -> prepare("SELECT * FROM solicitudes WHERE area = 'ADMINISTRATIVA' AND estado_JF = 'Pendiente'") ;
                ...
                <div class="box-body">
                  <table id="example1" class="table table-bordered">
                    ...
                    <tbody>
                      <?php
                         global $row;
                         $leaves = mysqli_query($conn,"SELECT * FROM solicitudes  WHERE area = 'ADMINISTRATIVA' AND estado_JF = 'Pendiente' AND estado_DA = 'Pendiente'");
                         if($leaves){
                         ...
                         while($row1 = mysqli_fetch_array($leaves)){
                           ...
                           <td>
                          <a class='btn btn-primary btn-sm editar btn-flat' href="ver_solicitud_administrativa.php?id=<?php echo $row1['id']?>"><i class="fa fa-commenting-o" aria-hidden="true"></i> Añadir Comentario</a>
                          <br>
                          <button type="button" class="btn btn-link"><a href="<?php echo $row1['documento']; ?>"> <i class="fa fa-download" aria-hidden="true"></i>Descargar Archivo</a></button></td>
                           ...
                           <td>
                            <?php echo"  
                          <a href=\"UpdateStatusAdministrativa.php?id={$row1['id']}&descripcion={$row1['descripcion']}\"><button class='btn-success btn-sm' >Aceptar</button></a>"?>
                          <a class='btn btn-danger btn-sm rechazar btn-flat' href="rechazar_administrativa.php?id=<?php echo $row1['id']?>" > Rechazar </a>
                        </td>
                          </tr>
                          <?php 
                           ;
